...
const GenerateMarksheet = () => {
  ...
  const fetchStudents = useCallback(async () => {
    try {
      ...
      // The API returns students directly as an array
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Calculate totals for the marksheet
      const totalMarks = subjects.reduce((sum, subject) => sum + (subject.fullMarks || 0), 0);
      const obtainedMarks = subjects.reduce((sum, subject) => sum + (subject.obtainedMarks || 0), 0);
      const percentage = totalMarks > 0 ? Math.round((obtainedMarks / totalMarks) * 100) : 0;
      ...
      // Add _id for update operations
      if (editingMarksheet) {
        marksheetData._id = editingMarksheet._id;
      }
      
      const method = editingMarksheet ? 'PUT' : 'POST';
      const url = '/api/admin/marksheets';
      ...
      if (result.success) {
        setSuccess(editingMarksheet ? 'Marksheet updated successfully!' : 'Marksheet generated successfully!');
        ...
        setError(result.error || 'Failed to save marksheet');
      }
    } catch (error) {
      console.error('Error saving marksheet:', error);
      setError('Error saving marksheet. Please check your connection.');
    } finally {
      setIsLoading(false);
    }
  }, [selectedStudent, examTitle, examType, examDate, subjects, rank, fetchStudentMarksheets, editingMarksheet]);
  ...
  const handleEdit = useCallback((marksheet: Marksheet) => {
    try {
      ...
      setExamDate(marksheet.examDate);
            ...
            {selectedStudent ? (
              <>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  >
                    {isLoading ? (editingMarksheet ? 'Updating...' : 'Generating...') : (editingMarksheet ? 'Update Marksheet' : 'Generate Marksheet')}
                  </button>
                </form>
